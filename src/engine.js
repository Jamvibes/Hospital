import {PATIENTS, STAFF, FACILITIES, MARKET} from './data.js?v=5';
...
export function createGame(seed=null){
  ...
  const state={round:1,phase:'assignment',money:10,reputation:5,deck:shuffle(clone(PATIENTS),resolvedSeed),discard:[],facilities:[],staff:[],market:[],resources:{medication:0,surgery:0},log:[],resolutionEvents:[],gameOver:false,nextId:1,rngState:resolvedSeed};
...
export function buy(state,kind,key){const offerIndex=state.market.findIndex(card=>card.kind===kind&&card.key===key),def=kind==='staff'?STAFF[key]:FACILITIES[key];if(state.phase!=='purchasing'||offerIndex<0||!def||state.money<def.cost)return false;state.money-=def.cost;if(kind==='staff')addStaff(state,key);else addFacility(state,key);state.market.splice(offerIndex,1);state.log.unshift(`${def.name} purchased for $${def.cost}.`);return true}
export function advancePhase(state){
  ...
  if(state.phase==='purchasing'){
    ...
    drawPatients(state,Math.min(2+Math.floor((state.round-1)/4),3));refreshMarket(state);state.phase='assignment';
