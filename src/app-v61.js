import {createGame,investigate,treat,admit,buy,advancePhase,assignStaff,returnStaff,placeFacility,moveFacility,compatible,getFacility,previewResolution,patientRisk,scheduleSurgery,cancelSurgery,placePostoperativePatient,surgeryEligibility,theatreCapacity,purchaseCost,upkeepCost,unmetNeeds,canEnterWard,radiologyInvestigate,hospitalHomeDischarge,appealLevel,staffOfferCount,CAMPUS,campusCell,builtFacilityCount} from './engine-v59.js';
import {STAFF,FACILITIES,STAFF_GROUPS} from './data.js?v=17';
import {renderMarket} from './ui/market.js';
import {SAVE_KEY,createSaveRecord,restoreSaveRecord} from './systems/save.js';

const storedRecord=readStoredRecord();
const restoredGame=restoreSaveRecord(storedRecord);
let game=restoredGame||createGame(),selectedStaff=null,selectedAdmission=null,selectedFacility=null,selectedAbility=null,selectedFacilityAbility=null,selectedSurgery=null,resolutionAnimating=false,rulesOpen=false,mapCompact=false,resolutionReport=null,saveDialog=restoredGame?'resume':null,lastSavedAt=restoredGame?storedRecord?.savedAt:null;
...
function render(){
  ...
  const mode=selectedAdmission?'admission':selectedFacility?'building':selectedAbility||selectedFacilityAbility?'ability':selectedSurgery?'surgery':selectedStaff?'staff':null,phase=phaseCopy[game.phase],preview=game.phase==='operations'&&previewResolution(game);
  $('stats').innerHTML=stat('Round',game.round)+stat('Stage',phase.name)+stat('Queue',game.queue.length)+stat('Reputation',game.reputation)+stat('Appeal',appealLevel(game))+stat('Money','$'+game.money)+stat('Next upkeep','$'+upkeepCost(game));
  ...
  $('briefing').innerHTML=selectedAdmission
    ...
    :selectedFacility?facilityPlacementBriefing()
    ...
    :selectedFacilityAbility?`<div><strong>Use ${FACILITIES[getFacility(game,selectedFacilityAbility).key].name}</strong><span>Choose one highlighted eligible patient in Emergency or a ward.</span><button data-action="cancelFacilityAbility">Cancel</button></div>`
  ...
  $('hospitalMap').className=`hospital-map ${mapCompact?'campus-compact':''}`;
  $('hospitalMap').innerHTML=Array.from({length:CAMPUS.columns*CAMPUS.rows},(_,cell)=>{const f=game.facilities.find(x=>campusCell(x.position)===cell);return f?facilityTile(f):buildPlot(cell)}).join('');
  $('campusCount').textContent=`${builtFacilityCount(game)}/${CAMPUS.maxFacilities} facilities · ${CAMPUS.columns*CAMPUS.rows} cells`;
  $('mapScaleToggle').textContent=mapCompact?'Standard view':'Compact view';
  ...
  $('endTurn').disabled=resolutionAnimating||game.gameOver||game.gameWon||Boolean(mode)||game.phase==='postoperative'||(game.phase==='purchasing'&&game.facilities.some(f=>!f.position));
  ...
  $('resolutionReport').hidden=!resolutionReport;
  $('resolutionReport').innerHTML=resolutionReport?renderResolutionReport(resolutionReport):'';
  $('savePanel').hidden=!saveDialog;
  $('savePanel').innerHTML=saveDialog?renderSaveDialog():'';
  $('saveStatus').textContent=lastSavedAt?'Saved':'Not saved';
  bind();
  if(!saveDialog&&!resolutionAnimating)persistGame();
}
...
function persistGame(){
  try{const record=createSaveRecord(game);localStorage.setItem(SAVE_KEY,JSON.stringify(record));lastSavedAt=record.savedAt;$('saveStatus').textContent='Saved'}
  catch{$('saveStatus').textContent='Save unavailable'}
}
function clearSelections(){selectedStaff=selectedAdmission=selectedFacility=selectedAbility=selectedFacilityAbility=selectedSurgery=null}
function beginNewGame(){localStorage.removeItem(SAVE_KEY);game=createGame();clearSelections();resolutionReport=null;saveDialog=null;lastSavedAt=null;render()}
function renderSaveDialog(){
  if(saveDialog==='resume')return `<section class="save-card"><span class="eyebrow">SAVED HOSPITAL FOUND</span><h2>Continue your hospital?</h2><p>Round ${game.round} · ${phaseCopy[game.phase]?.name||game.phase} · ${game.reputation} Reputation · $${game.money}</p><div class="save-actions"><button class="primary" data-action="continueGame">Continue</button><button class="secondary" data-action="confirmNewGame">Start new game</button></div></section>`;
  return `<section class="save-card"><span class="eyebrow">START AGAIN</span><h2>Begin a new hospital?</h2><p>Your current campaign will be replaced by a new random game.</p><div class="save-actions"><button class="danger-button" data-action="confirmNewGame">Start new game</button><button class="secondary" data-action="cancelNewGame">Keep playing</button></div></section>`
}
function renderResolutionReport(report){
  const rows=report.events.length?report.events.map(event=>{
    const facility=getFacility(game,event.facilityId),location=facility?FACILITIES[facility.key].name:'Hospital';
    if(event.type==='discharge')return `<li class="report-success"><span class="report-icon">✓</span><div><b>Patient ${event.portrait} discharged</b><small>${location}</small></div><strong>+$${event.reward} · +${event.reputation} rep</strong></li>`;
    if(event.type==='deteriorate')return `<li class="report-warning"><span class="report-icon">!</span><div><b>Patient ${event.portrait} deteriorated</b><small>${event.hidden?'A hidden need was added':`Added ${names[event.need]}`}</small></div><strong>${event.unmet} unmet before change</strong></li>`;
    if(event.type==='protected')return `<li class="report-protected"><span class="report-icon">◆</span><div><b>Patient ${event.portrait} protected</b><small>Intensive Care prevented deterioration</small></div><strong>No new need</strong></li>`;
    return `<li class="report-danger"><span class="report-icon">×</span><div><b>Patient ${event.portrait} died</b><small>${event.hidden?'Risk was hidden':`${event.unmet} unmet needs`}</small></div><strong>-${event.reputationLoss} rep</strong></li>`
  }).join(''):`<li class="report-neutral"><span class="report-icon">—</span><div><b>No patient outcomes</b><small>No patients discharged, deteriorated, or died.</small></div></li>`;
  const discharged=report.events.filter(e=>e.type==='discharge').length,deteriorated=report.events.filter(e=>e.type==='deteriorate').length,protectedCount=report.events.filter(e=>e.type==='protected').length,deaths=report.events.filter(e=>e.type==='death').length;
  return `<section class="resolution-report-card"><header><div><span class="eyebrow">PATIENT RESOLUTION</span><h2>Round ${report.round} outcomes</h2></div><button data-action="closeResolutionReport">Continue</button></header><div class="report-totals"><span><b>${discharged}</b> discharged</span><span><b>${deteriorated}</b> deteriorated</span><span><b>${protectedCount}</b> protected</span><span><b>${deaths}</b> deaths</span></div><ul>${rows}</ul><footer><div><small>Money change</small><b class="${report.moneyDelta>=0?'positive':'negative'}">${report.moneyDelta>=0?'+':''}$${report.moneyDelta}</b></div><div><small>Reputation change</small><b class="${report.reputationDelta>=0?'positive':'negative'}">${report.reputationDelta>=0?'+':''}${report.reputationDelta}</b></div><button class="primary" data-action="closeResolutionReport">Continue to Patient Resolution</button></footer></section>`
}
...
function rulesGuide(){
  ...
  <div class="rules-grid">
    ...
    <article><h3>Appeal and recruitment</h3><p>Appeal represents how attractive the hospital is as a workplace. It is not spent. Reveal 2 staff candidates at 0–2 Appeal, 3 at 3–5 Appeal, and 4 at 6+ Appeal. Facility offers are unaffected.</p></article>
    <article><h3>Special facilities</h3><p>Radiology investigates one patient anywhere each round. Rehabilitation accepts patients whose remaining needs are Nursing and/or Medication, but not Surgery. A Helipad adds one revealed Complex arrival each round. Hospital in the Home can discharge one investigated patient with at most one unmet need for their full reward.</p></article>
    <article><h3>Hospital development</h3><p>The Café and Volunteers generate Money each round. Staff Lounges and Hospital Libraries provide Appeal. Each Walk-in Centre adds one investigated Quick patient at round start.</p></article>
  ...
  <ol class="rules-stages">
    <li><b>Round start</b><span>Scheduled Surgery resolves. Postoperative patients must receive ward beds. Pharmacists generate Medication, income cards generate Money, and additional facility arrivals enter. Standard patient arrivals increase every four rounds: 2 in rounds 1–4, 3 in rounds 5–8, and 4 in rounds 9–12.</span></li>
    ...
    <li><b>Purchasing</b><span>Pay upkeep, then spend the remaining Money on available staff and facilities. Appeal determines whether 2, 3, or 4 staff candidates appear. Place new facilities on empty map plots and purchased staff into compatible vacant slots.</span></li>
  </ol>
...
function facilityTile(f){
  ...
  const beds=d.beds?Array.from({length:d.beds},(_,i)=>bed(f,f.patients[i],i)).join(''):d.kind==='theatre'?theatreSpaces(f,capacity):`<div class="equipment ${f.key}"><div class="equipment-core">${d.short}</div><span>${d.name}</span></div>`;
  const selectedMember=game.staff.find(s=>s.id===selectedStaff&&(game.phase==='operations'||(game.phase==='purchasing'&&s.purchasedRound===game.round))),selectedRole=selectedMember&&STAFF[selectedMember.key].role,selectedCanMove=selectedMember&&!selectedMember.used;
  const slots=d.slots.map(role=>{
    ...
    if(selectedCanMove&&role===selectedRole&&compatible(game,selectedStaff,f.id))return `<button class="staff-slot assignment-target" data-action="assign" data-staff="${selectedStaff}" data-facility="${f.id}" title="Move ${STAFF[selectedMember.key].name} here"><span>+</span><small>Move here</small></button>`;
    if(selectedCanMove)return `<div class="staff-slot unavailable-target" title="${role===selectedRole?`${roleNames[role]} slot unavailable`:`${STAFF[selectedMember.key].name} requires a ${roleNames[selectedRole]} slot`}"><span>×</span><small>${role===selectedRole?'Unavailable':`${roleNames[role]} only`}</small></div>`;
    return `<div class="staff-slot"><span>${(roleNames[role]||role).split(' ').map(x=>x[0]).join('')}</span><small>${roleNames[role]||role} slot</small></div>`;
  }).join('');
  return `<article class="facility facility-${f.key} ${d.colour} ${illustrated?'illustrated-facility':''}" data-facility="${f.id}" data-grid-x="${f.position.x}" data-grid-y="${f.position.y}"><header><div><span class="room-code">${d.short}</span><h3>${d.name}</h3></div><span class="occupancy">${d.beds?`${f.patients.length}/${d.beds} beds`:d.kind==='theatre'?`${f.patients.length}/${capacity} occupied`:`cell ${f.position.x+1},${f.position.y+1}`}</span></header><div class="room-art"><div class="floor-lines"></div><div class="beds">${beds}</div>${illustrated?`<div class="art-staff-slots" aria-label="Ward staff">${slots}</div>`:`<div class="station"><div class="desk"></div><small>${d.kind==='ward'?'Nurse station':d.kind==='clinical'?'Assessment desk':d.kind==='theatre'?'Theatre team':'Work area'}</small></div>`}</div><div class="room-footer">${illustrated?'':`<div class="slots">${slots}</div>`}<div class="facility-status">${facilityStatus(f,assigned,capacity)}</div>${facilityAbilityControl(f)}<div class="room-actions"><small>${d.effect}</small><div class="room-action-buttons"><span class="upkeep-label">${f.funded?'Funded':`$${d.upkeep} upkeep`}</span>${game.phase==='purchasing'?`<button class="relocate-button" data-action="relocateFacility" data-facility="${f.id}">Relocate</button>`:''}</div></div></div></article>`;
}
...
function facilityStatus(f,assigned,capacity){
  ...
  if(f.key==='rehabilitation')chips.push('<span class="ability"><b>REH</b> Nursing / Medication only</span>');
  if(f.key==='radiology')chips.push(`<span class="ability"><b>${f.abilityUsed?'✓':'1'}</b> ${f.abilityUsed?'used':'investigation'}</span>`);
  if(f.key==='helipad')chips.push('<span class="ability"><b>+1</b> Complex arrival</span>');
  if(f.key==='hospitalHome')chips.push(`<span class="ability"><b>${f.abilityUsed?'✓':'1'}</b> ${f.abilityUsed?'used':'home discharge'}</span>`);
  if(d.income)chips.push(`<span class="ability"><b>+$${d.income}</b> each round</span>`);
  if(d.appeal)chips.push(`<span class="ability"><b>+${d.appeal}</b> Appeal</span>`);
  if(d.quickArrivals)chips.push(`<span class="ability"><b>+${d.quickArrivals}</b> investigated Quick arrival</span>`);
  if(f.key==='ward')chips.push('<span class="neutral"><b>—</b> no special ability</span>');
  return chips.join('')
}

function facilityAbilityControl(f){
  if(!['radiology','hospitalHome'].includes(f.key))return '';
  const enabled=game.phase==='operations'&&!f.abilityUsed&&game.facilities.some(location=>FACILITIES[location.key].kind!=='theatre'&&location.patients.some(patient=>canFacilityTarget(f.id,patient,location)));
  const label=f.key==='radiology'?'Investigate any patient':'Discharge patient home';
  return `<button class="facility-ability-button" data-action="startFacilityAbility" data-facility="${f.id}" ${enabled?'':'disabled'}>${f.abilityUsed?'Ability used':enabled?label:'No eligible patients'}</button>`
}
...
function bed(f,p,i){
  if(!p){const target=selectedAdmission&&canEnterWard(findPatient(selectedAdmission),f),action=game.phase==='postoperative'?'placePostoperative':'admit';return target?`<button class="bed empty admission-target" data-action="${action}" data-id="${selectedAdmission}" data-target="${f.id}"><div class="pillow"></div><span>Place in bed ${i+1}</span></button>`:`<div class="bed empty"><div class="pillow"></div><span>Bed ${i+1}</span></div>`}
  ...
  if(selectedFacilityAbility&&canFacilityTarget(selectedFacilityAbility,p,f)){const facility=getFacility(game,selectedFacilityAbility),label=facility.key==='radiology'?'Investigate':'Discharge home';return `<button class="bed occupied ability-target" data-action="useFacilityAbility" data-facility="${facility.id}" data-id="${p.id}"><div class="pillow"></div>${patientVisual(p,'patient-bed-art')}<div class="bed-needs">${needs}</div><span class="target-label">${label}</span></button>`}
  ...
  if(game.phase==='operations'&&f.key==='ed')actions+=`<button data-action="startAdmission" data-id="${p.id}" ${hasVacantWard(p)?'':'disabled'}>Admit to ward</button>`;
...
function bind(){document.querySelectorAll('[data-action]').forEach(b=>b.onclick=e=>{e.stopPropagation();const x=b.dataset,a=x.action;let ok=true;
  if(a==='saveGame'){persistGame();toast('Hospital saved.');return}
  if(a==='continueGame'){saveDialog=null;render();return}
  if(a==='openNewGame'){saveDialog='new';render();return}
  if(a==='cancelNewGame'){saveDialog=null;render();return}
  if(a==='confirmNewGame'){beginNewGame();return}
  if(a==='closeResolutionReport'){resolutionReport=null;render();return}
  if(a==='toggleMapScale'){mapCompact=!mapCompact;render();return}
  if(a==='openRules'){rulesOpen=true;render();return}
  if(a==='closeRules'){rulesOpen=false;render();return}
  if(a==='restartGame'){beginNewGame();return}
  ...
  if(a==='startFacilityAbility'){selectedFacilityAbility=x.facility;selectedStaff=selectedAdmission=selectedAbility=null;render();return}
  if(a==='cancelFacilityAbility'){selectedFacilityAbility=null;render();return}
  if(a==='relocateFacility'){selectedFacility=x.facility;selectedStaff=selectedAdmission=selectedAbility=selectedFacilityAbility=selectedSurgery=null;render();return}
  if(a==='cancelFacility'){const f=getFacility(game,selectedFacility);if(f&&!f.position){game.money+=f.purchasePrice??FACILITIES[f.key].cost;if(f.usedAdministratorDiscount)game.facilityDiscountUsed=false;game.market.push({kind:'facility',key:f.key});game.facilities.splice(game.facilities.indexOf(f),1)}selectedFacility=null;render();return}
  ...
  else if(a==='placeFacility'){ok=placeFacility(game,x.facility,Number(x.cell));if(ok)selectedFacility=null}
  else if(a==='moveFacility'){ok=moveFacility(game,x.facility,Number(x.cell));if(ok)selectedFacility=null}
  ...
  else if(a==='useFacilityAbility'){const facility=getFacility(game,x.facility);ok=facility?.key==='radiology'?radiologyInvestigate(game,facility.id,x.id):facility?.key==='hospitalHome'?hospitalHomeDischarge(game,facility.id,x.id):false;if(ok)selectedFacilityAbility=null}
  ...
  else if(a==='buy'){ok=buy(game,x.kind,x.key);if(ok&&x.kind==='facility')selectedFacility=game.facilities.find(f=>!f.position)?.id||null;if(ok&&x.kind==='staff'&&!STAFF[x.key].hospitalWide)selectedStaff=[...game.staff].reverse().find(s=>s.purchasedRound===game.round&&!s.facilityId&&!STAFF[s.key].hospitalWide)?.id||null}
...
function hasVacantWard(patient){return game.facilities.some(f=>f.position&&canEnterWard(patient,f))}
function canFacilityTarget(facilityId,patient,location){const facility=getFacility(game,facilityId);if(game.phase!=='operations'||!facility||facility.abilityUsed||FACILITIES[location.key].kind==='theatre')return false;if(facility.key==='radiology')return !patient.revealed;if(facility.key==='hospitalHome')return patient.revealed&&unmetNeeds(patient)<=1;return false}
...
function hasFreePlot(){return builtFacilityCount(game)<CAMPUS.maxFacilities}
...
$('endTurn').onclick=async()=>{
  ...
  if(game.phase==='operations'){
    const rects=capturePatientRects(),before={money:game.money,reputation:game.reputation,round:game.round};resolutionAnimating=true;
    if(!advancePhase(game)){resolutionAnimating=false;return}
    clearSelections();render();
    await playResolutionEvents(game.resolutionEvents,rects);resolutionAnimating=false;
    resolutionReport={round:before.round,events:game.resolutionEvents.map(event=>({...event})),moneyDelta:game.money-before.money,reputationDelta:game.reputation-before.reputation};
    render();return
  }
  if(!advancePhase(game))toast('Place every purchased facility before starting the next round.');
  clearSelections();render()
};
$('reset').onclick=()=>{saveDialog='new';render()};
